...
class Response {
  ...
  flow (config) {
    let flow = this.session('alexa-router-flow')
      ? JSON.parse(this.session('alexa-router-flow'))
      : []

    if (Array.isArray(config)) {
      config.forEach(config => this.flow(config))
    } else {
      ...
      helpers.checkConflictingFlow(config, flow)

      flow.push(config)
      this.session('alexa-router-flow', JSON.stringify(flow))
    }
  }
